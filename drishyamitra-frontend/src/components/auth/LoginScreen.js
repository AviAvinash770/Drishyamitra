...
export default function LoginScreen({ onLoginSuccess }) {
  ...
  const handleGoogleLoginResponse = async (response) => {
    ...
    try {
      ...
      await api.auth.googleVerify(idToken);
      onLoginSuccess();
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const loadGoogleClientId = async () => {
      try {
        ...
        console.error("Failed to load Google configuration or initialize Google client:", err);
      }
    };
    loadGoogleClientId();
      ...
      <div style={{
        ...
        {!googleInitialized ? (
          <button
            type="button"
            onClick={() => {
              if (googleClientIdMissing) {
                setError("Google Client ID is missing. Please configure GOOGLE_CLIENT_ID in your backend .env file and restart the server.");
              } else {
                setError("Loading Google Sign-in...");
              }
            }}
            style={{
              padding: "12px",
